---
title: Apache Web Server Installation and SSL Configuration
toc: Set Up Apache
description: Install Apache on Debian/Ubuntu or RHEL/CentOS, configure SSL with Let's Encrypt Certbot, set up virtual hosts, and enable security headers.
---
<Tabs defaultValue="debian" className="pt-5 pb-1">
  ...
  <TabsContent value="debian">
    ...
    1) Create a new Apache site configuration like `website.conf` in `/etc/apache2/sites-available` using a text editor like nano or vim.
    2) Paste the text below into the file, replacing `<domain>` with your domain:
    ```apacheconf:website.conf
    <VirtualHost *:80>
      ServerName <domain>
    ...
    <StepperItem title="Link Config and Restart Apache">
