---
title: How To Install WHMCS Billing Software
toc: Install WHMCS
description: Full WHMCS installation guide with Apache, PHP 8.1, IonCube loader, MariaDB database setup, SSL certificates, and cron job configuration.
---

<Stepper>
<StepperItem title="Download WHMCS">
...
<StepperItem title="Install Apache">
```bash
apt install apache2
```

</StepperItem>
<StepperItem title="Generating SSL/TLS Certificates">
#### Downloading Certbot
...
<StepperItem title="Automatically Renewing SSL/TLS certificates">
1) Install crontab:
```bash
apt install cron
```
...
<StepperItem title="Webserver Configuration">
...
```apacheconf:whmcs.conf showLineNumbers {2,23,36,37}
<VirtualHost *:80>
...
<StepperItem title="Install IonCube">
```bash
...
```ini:/etc/php/8.1/apache2/php.ini
zend_extension = "/usr/lib/php/20210902/ioncube_loader_lin_8.1.so"
```
...
<StepperItem title="Enable Required Modules">
```bash
a2enmod rewrite
a2enmod headers
a2enmod ssl
...
<StepperItem title="Restart Apache and PHP">
```bash
...
<StepperItem title="Install MariaDB">
```bash
...
<StepperItem title="Enter MySQL Shell">
```bash
mysql
```

</StepperItem>
<StepperItem title="Create Database">
```sql
...
<StepperItem title="Setting File Permissions">
```bash
chown -R www-data:www-data /var/www/whmcs
```

</StepperItem>
<StepperItem title="Create Cron Schedule">
```bash
(crontab -l ; echo "*/5 * * * * /usr/bin/php -q /var/www/crons/cron.php")| crontab -
```

</StepperItem>
<StepperItem title="Begin Setup">
