---
title: How To Configure MySQL for Remote Access
toc: Remote MySQL Access
description: Configure MySQL for remote access by creating users, setting bind-address in my.cnf, and opening firewall ports. Works with UFW, iptables, and nftables.
---

## Creating a MySQL user for a remote IP
For specifics on how [MySQL users](/docs/mysql/setting-up-mysql#creating-a-mysql-user) work and how [MySQL privileges](/docs/mysql/setting-up-mysql#setting-mysql-user-privileges) work, follow the corresponding guides but this example, is using the MySQL shell to allow access to the remote IP `192.168.53.72`.
```bash
...
<Tabs defaultValue="ufw" className="pt-5 pb-1">
  <TabsList>
    <TabsTrigger value="ufw">UFW</TabsTrigger>
    <TabsTrigger value="iptables">IPTables</TabsTrigger>
    <TabsTrigger value="nftables">NFTables</TabsTrigger>
  </TabsList>
  <TabsContent value="ufw">
