...
3) Add your servers under the servers section. For example, let's add a server called "Hub" with the IP address 172.18.0.1:30000 and a MOTD of "Pinging...". You can modify the server details as per your requirements.
...
```
6) Enable IP forwarding by setting the `ip_forward` option to `true`. This allows BungeeCord to forward player connections to the appropriate Minecraft server.
```yaml filename="config.yml" copy
ip_forward: true
```
7) Save the `config.yml` file.
### Firewall Configuration
If you want to forward player connections to the BungeeCord network, you'll need to configure a firewall rule to allow incoming connections to the BungeeCord IP address and port. Here's an example of how to do it using the ufw firewall utility:
...
```
The firewall rule is now set up to allow incoming connections to the BungeeCord IP address and port range.
