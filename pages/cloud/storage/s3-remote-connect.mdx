...
```
The following script is an example of how to backup example.txt from /root, all mysql databases, and all virtual machines:
...
# Backup /root/1.txt
rclone copy --dedupe-mode interleaved --bwlimit 60M "/root/1.txt" storage:files/${node_id}/$(date +%F)/
...
if command -v mysqldump &> /dev/null; then
  mysql -s -r -e 'show databases' | grep -vE '^(information_schema|performance_schema|mysql|sys)$' | while read db; do
    ...
    rclone copy --dedupe-mode interleaved --bwlimit 60M "$tmp_dir/mysqldump/${db}.sql" storage:mysqldump/${node_id}/$(date +%F)/
...
if command -v virsh &> /dev/null; then
  virsh list | grep kvm | awk '{print $2}' | while read vm; do
    ...
    rclone copy --dedupe-mode interleaved --bwlimit 60M "$tmp_dir/virtual/${vm}_backup.gz" storage:virtual/${node_id}/$(date +%F)/
