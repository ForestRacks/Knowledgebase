...
GRANT ALL PRIVILEGES *.* TO 'your_user'@'%' WITH GRANT OPTION;
```
...
<Steps>
### Find Config
The MySQL configuration file usually located at `/etc/my.cnf` but if not, type `find /etc -iname my.cnf` to locate it.
### Open Config
