...
```
6) Click "Save"
7) Repeat the same steps for `selector2`
...
1) Go to your DNS Provider's Portal
2) Click "Add Record" on the DNS page
3) Select a `CNAME-Record`
4) Set the name as `selector1._domainkey`
5) Set the value as `selector1-example-com._domainkey.examplecom.onmicrosoft.com` replacing `example-com` and `examplecom` with your corrosponding domain.
6) Repeat the same steps for `selector2`