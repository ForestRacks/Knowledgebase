...
* A Linux server or virtual machine (VM) running a compatible distribution (e.g., Ubuntu, Debian).
* Root access to the VPS or Dedicated Server.

<Steps>
### Install WireGuard
1) Connect to your server/VM via SSH using your preferred terminal emulator or command prompt.
...
3) Copy the public key to the server/VM:
```bash copy
